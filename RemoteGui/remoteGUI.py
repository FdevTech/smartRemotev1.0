...
keyboard = Controller()
def remote(root,dev2,var2):
    global device ,variable
    device=dev2
    variable=var2
    window = Toplevel(root)
    window.title('Remote Ccntrol')
    window.geometry("500x550+500+100")
    ...
    # Row 10
    buttonEnd = Button(MainFrame, text='End recording ', width=15, height=2, bg='medium turquoise', fg='black', command=end2)
    buttonEnd.grid(row=9, column=2, padx=5, pady=5)
...
def end2 ():
    ...
    os.system(exec1)
    time.sleep(2)
    keyboard.type("sudo cat "+device+".lircd.conf>> /etc/lirc/lircd.conf")
    keyboard.press(Key.enter)
    keyboard.release(Key.enter)
    keyboard.type("exit")
    keyboard.press(Key.enter)
